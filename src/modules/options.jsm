...
function saveTranslations() {
	let out = '';
	let list = new Array();
	let entries = translationsList.getElementsByTagName('listitem');
	for(let i = 0; i < entries.length; i++){
		let columns = entries[i].getElementsByTagName('listcell');
...
function callbackLanguages(responseText) {
	...
	let hrefPrefixShort = './?action=buildup&targetlang=';
	let hrefPrefixLong = 'http://contribute.dict.cc/?action=buildup&targetlang=';
	while(responseText.indexOf(hrefPrefixShort) != -1)
		responseText = responseText.replace(hrefPrefixShort, hrefPrefixLong);

	let htmlDoc = Helpers.parseHTML(document, responseText);
	let elements = htmlDoc.getElementsByTagName("a");
	let list = new Array();
	for(let i=0; i<elements.length; i++) {
		if(elements[i].href.indexOf(hrefPrefixLong) == 0) {
			let lang = elements[i].href.substring(hrefPrefixLong.length);
...
var SearchDictCC_Options = {
	...
	onLoad : function () {
		...
		let menu = document.getElementById("firstLanguagePopup");
	...
	onAccept : function () {
		let result = SearchDictCC_Options.onBeforeAccept();
		if(!result) {
	...
	onAdd : function () {
		let firstLanguage = document.getElementById("firstLanguage").selectedIndex;
		let secondLanguage = document.getElementById("secondLanguage").selectedIndex;
		let direction = document.getElementById("languageDirection").selectedIndex;
		...
		if(secondLanguage == 0) {
			...
			let languages = Prefs.getLanguages();
			let first;
			if(firstLanguage == 0)
			...
			let second = languages[secondLanguage-1];
	...
	onManualAdd : function () {
		let subdomain = prompt(strings.GetStringFromName("enterSubdomain"), "www");
		if(!subdomain) {
			return;
		}
		let label = prompt(strings.GetStringFromName("enterLabel"), "DE=>EN");
		if(!label) {
			return;
		}
		addEntry(label, subdomain);
	...
	onRemove : function () {
		let entry = translationsList.selectedItem;
		if(entry) {
			let columns = entry.getElementsByTagName('listcell');
	...
	onEdit : function () {
		...
		if(entry) {
			let columns = entry.getElementsByTagName('listcell');
			let label = prompt(strings.GetStringFromName("enterLabel"), columns[0].getAttribute('label'));
			if(!label) {
				return;
			}
	...
	onMoveUp : function () {
		...
		if(entry && entry.previousSibling && entry.previousSibling.tagName == 'listitem') {
			let before = entry.previousSibling;
	...
	updateSecondLanguageList : function() {
		let parentMenu = document.getElementById("secondLanguage");
		let menu = document.getElementById("secondLanguagePopup");
		let selection = '';
		...
		while(menu.childNodes.length > 0) {
			let node = menu.childNodes[menu.childNodes.length-1];
