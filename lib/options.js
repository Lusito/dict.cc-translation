...
function upgradeSerializationToJson(key) {
    var value = simplePrefs.prefs[key];
    if(!value.contains('{|}'))
        return;
    var result = [];
    ...
    for(var i=0; i<parts.length; i++) {
        ...
        if(entry.length === 2) {
            result.push({k: entry[0], v: entry[1]});
        }
    }
    simplePrefs.prefs[key] = JSON.stringify(result);
}
//fixme: before release, make package.json contain corrected values
upgradeSerializationToJson("translation.list");
upgradeSerializationToJson("translation.languages");
...
exports.requestLanguageUpdate = function() {
    ...
    utils.requestHTML(url, function(doc) {
        ...
        optionsPanel.port.emit('languageListUpdate', list, null);
    }, function() {
        optionsPanel.port.emit('languageListUpdate', null, 'Error getting languages'); // fixme: translation
    });
};
