...
function resetPreferences() {
    for (var i = 0; i < preferencesList.length; i++) {
        var pref = preferencesList[i];
        if(pref.hasOwnProperty('value'))
            simplePrefs.prefs[pref.name] = pref.value;
    }
}
...
exports.show = function () {
    ...
    if (optionsPanel) {
        ...
        optionsPanel.contentURL = self.data.url('options.html');

        optionsPanel.port.on("init", function () {
            initialized = true;
            optionsPanel.port.emit('show', createPreferences());
        });
        optionsPanel.port.on("restore_defaults", function () {
            resetPreferences();
            optionsPanel.port.emit('show', createPreferences());
        });
