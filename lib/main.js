...
exports.main = function(options, callbacks) {
    // flush l10n on install, upgrad, downgrade
    var flushReasons = [ "install", "upgrade", "downgrade"];
    if(flushReasons.indexOf(options.loadReason) >= 0) {
        var chrome = require('chrome');
        chrome.Cc["@mozilla.org/intl/stringbundle;1"].getService(chrome.Ci.nsIStringBundleService).flushBundles();
    }
};